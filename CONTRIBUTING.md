...
- **First time contributors are FORBIDDEN to submit any code generated by LLMs.**
...
- Any submission (being done with LLMs or not) is the author's responsibility.
We consider any code submitted have **at the very least** been **carefully reviewed** by the author before submission.
...
- run `scripts/fmt_all.sh` on your code before pushing
- check the output of `./scripts/clippy.sh` (On windows use `.\scripts\clippy.ps1`)
