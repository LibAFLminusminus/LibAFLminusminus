...
impl TryFrom<&str> for Signal {
    type Error = libaflmm_core::Error;
    ...
    fn try_from(value: &str) -> Result<Self> {
        Ok(match value {
            ...
            _ => return Err(illegal_argument!("No signal named {value}")),
        })
    }
}
    ...
    unsafe {
        ...
        for sig in signals {
            ...
            if sigaction(sig as i32, &raw mut sa, ptr::null_mut()) < 0 {
                ...
                return Err(unknown!("Could not set up {sig} handler"));
            }
        }
...
#[inline(always)]
pub fn ucontext() -> Result<ucontext_t> {
    ...
    if cfg!(not(any(target_os = "openbsd", target_os = "haiku"))) {
        if unsafe { getcontext(&raw mut ucontext) } == 0 {
            ...
            Err(unknown!("Failed to get ucontext: {:?}", Errno::last()))
        }
    } else {
        Ok(ucontext)
    }
