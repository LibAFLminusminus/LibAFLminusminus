//! Map observer with a shrinkable size accessible through a pointer.
...
/// A [`SizePtrMapObserver`] overlooking a variable bitmap
#[derive(Serialize, Deserialize, Debug)]
#[expect(clippy::unsafe_derive_deserialize)]
pub struct SizePtrMapObserver<'a, T> {
...
impl<T> DependencyResolver for SizePtrMapObserver<'_, T> {}

impl<S, T> Observer<S> for SizePtrMapObserver<'_, T>
where
    Self: MapObserver,
{
    #[inline]
...
impl<T> Named for SizePtrMapObserver<'_, T> {
    #[inline]
...
impl<T> HasLen for SizePtrMapObserver<'_, T> {
    #[inline]
    fn len(&self) -> usize {
        *self.size.as_ref()
    }
...
impl<T> Hash for SizePtrMapObserver<'_, T>
where
    T: Hash,
{
    #[inline]
...
impl<T> AsRef<Self> for SizePtrMapObserver<'_, T> {
    fn as_ref(&self) -> &Self {
        self
    }
}

impl<T> AsMut<Self> for SizePtrMapObserver<'_, T> {
    fn as_mut(&mut self) -> &mut Self {
        self
    }
}

impl<T> MapObserver for SizePtrMapObserver<'_, T>
where
...
impl<T> VarLenMapObserver for SizePtrMapObserver<'_, T>
where
    T: PartialEq + Copy + Hash + Serialize + DeserializeOwned + Debug,
{
...
impl<T> Deref for SizePtrMapObserver<'_, T> {
    type Target = [T];
    fn deref(&self) -> &[T] {
...
impl<T> DerefMut for SizePtrMapObserver<'_, T> {
...
impl<'a, T> SizePtrMapObserver<'a, T>
where
    T: Default,
{
