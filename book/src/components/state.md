...
The Corpus is where `Testcase`s are stored. 
We define a `Testcase` structure to hold the input along with other minor metadata such as ids and file format specifiers.
...
Related to the Corpus is the way in which the next testcase (the fuzzer would ask for) is retrieved from the Corpus. 
The taxonomy for this handling in LibAFL is `Scheduler`, the entity representing the policy to fetch testcases from the Corpus.
...
`State` contains both corpus and objective corpus. 
Both are `Corpus` object.
