...
For instance, a `StdMutationalStage`, given an input of the corpus, applies a Mutator and executes the generated input for a couple of times. 
How many times this has to be done can be scheduled, AFL for instance uses a performance score of the input to choose how many times the havoc mutator should be invoked. 
This can depend also on other parameters, for instance, the length of the input if we want to just apply a sequential bitflip, or a fixed value.

Also, stages can be used to retrieve information by running the un-mutated input just for once. 
...
`SingleRunStage`, is used for running the un-mutated testcase once along with user-supplied hooks, and is useful for in-process cmplog observation.
