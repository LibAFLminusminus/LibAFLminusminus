...
For example, our `StdFuzzer` is a `Fuzzer` implementation for mutational fuzzing will first pick a testcase chosen by the scheduler, then run the testcase with all the provided [`Stages`](./stage.md).
Just simple as that.
